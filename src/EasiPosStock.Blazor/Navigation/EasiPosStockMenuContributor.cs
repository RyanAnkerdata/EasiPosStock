...
namespace EasiPosStock.Blazor.Navigation;

public class EasiPosStockMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public EasiPosStockMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<EasiPosStockResource>();

        context.Menu.AddItem(new ApplicationMenuItem(
            EasiPosStockMenus.Home,
            l["Menu:Home"],
            "/",
            icon: "fas fa-home",
            order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                EasiPosStockMenus.HostDashboard,
                l["Menu:Dashboard"],
                "/HostDashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(EasiPosStockPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                EasiPosStockMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "/Dashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(EasiPosStockPermissions.Dashboard.Tenant)
        );
